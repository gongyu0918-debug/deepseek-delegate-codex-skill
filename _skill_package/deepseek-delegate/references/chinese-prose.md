...
Load this file only when DeepSeek Delegate is being used for bounded Chinese prose review.

## Good Fits

- Short or chunked Chinese drafts where native wording, register, fluency, or ambiguity matters.
...
- Do not ask DeepSeek to rewrite a whole long document in one pass; chunk by section and ask for section-local findings.
- Do not treat DeepSeek as the final authority on law, medicine, finance, policy, current facts, or organization-specific truth.
...
- Text chunk: the smallest self-contained section needed.
...
- Merge duplicate wording findings across chunks before reporting.
- Keep the final user-facing edit consistent with the user's requested style, not DeepSeek's style by default.
